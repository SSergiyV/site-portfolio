(() => {
    ...
    class ScrollWatcher {
        ...
        scrollWatcherIntersecting(entry, targetElement) {
            if (entry.isIntersecting) {
                let elem = document.querySelector(".skills__content");
    ...
    window.onscroll = function() {
        let progressHeight = window.pageYOffset / fullHeight * 100;
        progress.style.height = progressHeight + "%";
    };
    const menuLinks = document.querySelectorAll(".run[data-goto]");
    ...
    const menuLinksAll = document.querySelectorAll(".menu__link");
    const classOpen = document.querySelectorAll(".menu-open");
    if (menuLinksAll && classOpen) menuLinksAll.forEach((res => {
    ...
    setInterval((() => {
        let time = new Date;
        let hr = 62.7 * time.getHours();
        let mn = 6 * time.getMinutes();
        let sc = 6 * time.getSeconds();
